...
export function ServiceReviews({ slug, serviceTitle }) {
    ...
    const avg = reviews.length > 0
        ...
        : null;
    return (<section className="reveal py-24 bg-surface">
      <div className="container-x grid lg:grid-cols-[1fr_1.2fr] gap-14">
        <div>
          ...
          {avg && (<div className="mt-6 inline-flex items-center gap-3 bg-white border border-border rounded-2xl px-5 py-3">
              <div className="text-3xl font-bold text-primary">{avg}</div>
              <div>
                <div className="flex text-primary text-sm">
                  {Array.from({ length: 5 }).map((_, i) => i < Math.round(Number(avg)) ? <FaStar key={i}/> : <FaRegStar key={i}/>)}
                </div>
          ...
          <form onSubmit={submit} className="mt-8 bg-white border border-border rounded-2xl p-6 space-y-4">
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Your Name</label>
              <input value={name} onChange={(e) => setName(e.target.value)} required className="mt-1 w-full rounded-lg border border-border bg-white px-4 py-2.5 text-sm focus:outline-none focus:border-primary" placeholder="e.g. Rohit Sharma"/>
            </div>
            <div>
            ...
            return (<button type="button" key={n} onMouseEnter={() => setHover(n)} onMouseLeave={() => setHover(0)} onClick={() => setRating(n)} className="hover:scale-110 transition-transform" aria-label={`${n} star${n === 1 ? "" : "s"}`}>
            ...
            <div>
              ...
              <textarea value={text} onChange={(e) => setText(e.target.value)} required rows={4} className="mt-1 w-full rounded-lg border border-border bg-white px-4 py-2.5 text-sm focus:outline-none focus:border-primary resize-none" placeholder="Tell others what worked well…"/>
            </div>
        ...
        <div>
          ...
          {reviews.length === 0 ? (<div className="mt-6 p-8 rounded-2xl border border-dashed border-border bg-white text-center text-muted-foreground">
              No reviews yet. Be the first to share your experience!
            </div>) : (<div className="mt-6 space-y-4 max-h-[640px] overflow-y-auto pr-2">
              {reviews.map((r) => (<div key={r.id} className="bg-white rounded-2xl p-6 border border-border shadow-[var(--shadow-soft)] animate-fade-in">
