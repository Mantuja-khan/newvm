...
export function Footer() {
  return (
    <footer className="relative mt-24 text-white bg-[image:var(--gradient-dark)] overflow-hidden">
      ...
      <div className="absolute top-6 left-8 w-28 h-28 dot-pattern-white opacity-25 rounded-xl pointer-events-none -rotate-6 hidden sm:block" />
      <div className="absolute top-1/2 right-10 -translate-y-1/2 w-36 h-36 dot-pattern-white opacity-20 rounded-2xl pointer-events-none rotate-12 hidden md:block" />
      <div className="absolute bottom-8 left-1/3 w-32 h-32 dot-pattern-white opacity-25 rounded-xl pointer-events-none rotate-3" />
        ...
        <div>
          <div className="flex items-center gap-2 mb-5">
            <img
              src={logoImg}
              alt={BRAND.name}
              className="h-16 w-auto object-contain bg-white rounded-xl p-2 shadow-lg"
            />
          </div>
          <p className="text-white/70 text-sm leading-relaxed mb-6">
            {BRAND.full} is a trusted technology partner delivering software, cloud, hardware and
            consulting solutions to modern businesses across India.
          </p>
          <div className="flex gap-3">
            {[FaFacebookF, FaTwitter, FaLinkedinIn, FaInstagram].map((Icon, i) => (
              <a
                ...
                href="#"
                aria-label="social"
                className="grid place-items-center h-9 w-9 rounded-full border border-white/20 hover:bg-primary hover:border-primary transition-colors"
              >
