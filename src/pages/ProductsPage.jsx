            ...
            <div key={cat.id} id={cat.id} className="mt-16 scroll-mt-24">
              ...
              <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {productList.filter((p) => p.cat === cat.id).map((p) => {
                  const itemLink = p.type === "hardware" ? `/hardware/${p.id}` : `/software/${p.id}`;
                  return (
                    <div key={p.name} className="group bg-white rounded-2xl p-4 shadow-[0_2px_12px_-3px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_-4px_rgba(0,0,0,0.08)] transition-all flex flex-col justify-between">
                      <div>
                        ...
                        {p.rating && (
                          <div className="flex items-center gap-1.5 mt-1.5">
                            <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-slate-800 text-[10px] font-bold text-white leading-none">
                              {p.rating} ★
                            </span>
                      ...
                      <div>
                        ...
                        <div className="mt-4 flex gap-2">
                          <Link to={itemLink} className="btn-primary w-full py-2 text-xs flex items-center justify-center gap-1">
                            View Product <FaArrowRight className="text-[10px]" />
                          </Link>
                        </div>
                      </div>
                    </div>
