...
import { FaArrowRight, FaCheckCircle, FaShieldAlt, FaLightbulb, FaHandshake, FaPlus, FaMinus, FaExternalLinkAlt, } from "react-icons/fa";
    ...
    return (<>
      ...
      <section className="reveal relative overflow-hidden bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('/hero-cta-bg.png')" }}>
        ...
        <div className="container-x relative pt-8 pb-16 md:pt-10 md:pb-24 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.1 }} className="relative group order-1 lg:order-2">
            ...
            <div className="absolute -top-32 -right-32 w-[600px] h-[600px] pointer-events-none mix-blend-screen z-10" style={{
            ...
            <div className="absolute -top-20 right-1/4 w-[160px] h-[450px] origin-top-right rotate-[40deg] pointer-events-none opacity-45 mix-blend-screen z-10" style={{
            ...
            <div className="relative w-full max-w-sm sm:max-w-md md:max-w-lg mx-auto overflow-hidden">
              <Swiper modules={[Autoplay, Pagination]} autoplay={{ delay: 4000, disableOnInteraction: false }} pagination={{ clickable: true }} loop spaceBetween={0} slidesPerView={1} onSlideChange={(sw) => setHeroIdx(sw.realIndex)} className="hero-swiper">
                {HERO_SLIDES.map((s) => (<SwiperSlide key={s.slug}>
                    <img src={s.image} alt={s.title} loading="eager" className="w-full h-[200px] sm:h-[300px] md:h-[480px] object-contain mx-auto"/>
                  </SwiperSlide>))}
              </Swiper>
            </div>
          </motion.div>
      ...
      <section className="reveal py-20 md:py-24 bg-surface overflow-hidden">
        <div className="container-x">
          ...
          <div className="mt-14">
            <Swiper modules={[Autoplay, Pagination]} autoplay={{ delay: 3500, disableOnInteraction: false }} pagination={{ clickable: true }} loop spaceBetween={24} breakpoints={{
            0: { slidesPerView: 1 },
            640: { slidesPerView: 2 },
            1024: { slidesPerView: 3 },
        }} className="!pb-14">
              {SERVICES.map((s) => (<SwiperSlide key={s.slug} className="h-auto">
                  <Link to="/services/$slug" params={{ slug: s.slug }} className="group relative flex flex-col h-full bg-white rounded-2xl p-8 shadow-[var(--shadow-soft)] hover:shadow-[var(--shadow-card)] border border-transparent hover:border-primary/20 transition-all overflow-hidden">
        ...
        <div className="container-x grid lg:grid-cols-2 gap-14 items-center">
          ...
          <div>
            ...
            <ul className="mt-8 space-y-3">
              {["24/7 On-site Services Available", "Great Skilled Consultants", "Expert Team Members", "Solution for Every Business Size"].map((f) => (<li key={f} className="flex items-center gap-3 text-sm font-medium">
      ...
      <section className="reveal py-24 bg-surface">
        <div className="container-x">
          ...
          <div className="mt-14 grid md:grid-cols-3 gap-6">
            {WHY.map((w, i) => (<motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.5 }} className="bg-white rounded-2xl p-8 border border-border hover:border-primary/30 transition-colors">
      ...
      <section className="reveal py-24">
        <div className="container-x">
          ...
          <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
            {PROCESS.map((p, i) => (<motion.div key={p.n} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="relative p-8 rounded-2xl bg-white border border-border hover:shadow-[var(--shadow-card)] transition-shadow">
...
function FAQList() {
    ...
    return (<div className="space-y-3">
      {FAQS.map((f, i) => (<div key={i} className={`rounded-xl border transition-all ${open === i ? "border-primary bg-white shadow-[var(--shadow-soft)]" : "border-border bg-white"}`}>
