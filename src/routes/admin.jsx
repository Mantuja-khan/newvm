...
function AdminPage() {
  ...
  const handleLogin = async (e) => {
    ...
    if (
      trimmedEmail === "vishalvmsolutiions@gmail.com" &&
      (trimmedPassword === "vishal@53990@" || trimmedPassword === "vishal@53990#")
    ) {
  ...
  const handleSaveModalProduct = () => {
    ...
    const finalProduct = {
      ...
      id:
        editProduct.id ||
        `${editProduct.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${Date.now().toString(36)}`,
    };
  ...
  const handleAddBusy = () => {
    const newBusyList = [
      ...busyPricing,
      { edition: "Basic", variant: "New Variant", activation: "₹0", renewal: "₹0" },
    ];
  ...
  const handleAddTally = () => {
    const newTallyList = [
      ...tallyPricing,
      { variant: "New Tally Variant", activation: "₹0", renewal: "₹0" },
    ];
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl border border-slate-100 shadow-md">
          <div className="text-center">
            ...
            <h2 className="mt-4 text-3xl font-extrabold text-slate-900 font-display">
              Admin Login
            </h2>
            <p className="mt-2 text-sm text-slate-500">
              Sign in to manage products & pricing charts
            </p>
          </div>
          {error && (
          ...
          <form className="mt-8 space-y-4" onSubmit={handleLogin}>
            <div>
              ...
              <input
                ...
                className="mt-1 w-full px-4 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
            ...
            <button
              type="submit"
              className="w-full btn-primary py-2.5 rounded-xl font-semibold mt-6"
            >
              Sign In
            </button>
          </form>
  ...
  const hardwareProducts = safeProducts.filter(
    (p) =>
      p &&
      (p.type === "hardware" ||
        ["laptops", "desktops", "workstations", "accessories"].includes(p.cat || "")),
  );
  const softwareProducts = safeProducts.filter(
    (p) =>
      p &&
      (p.type === "software" || p.type === "cloud" || ["software", "cloud"].includes(p.cat || "")),
  );
  return (
      ...
      <div className="container-x">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-slate-200">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Manage Hardware Products (Multiple Images, Price, MRP, Specs) & Software Catalog.
            </p>

            {/* MongoDB Connection Status Indicator */}
            {dbStatus && (
              <div
                className="mt-3.5 inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold border shadow-sm transition-all"
                style={{
              ...
              >
                <span
                  className={`h-2.5 w-2.5 rounded-full ${dbStatus.connected ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`}
                ></span>
                <span>
                  MongoDB Database:{" "}
                  {dbStatus.connected
                    ? `CONNECTED to Atlas (${dbStatus.host})`
                    : dbStatus.statusText}
                </span>
              </div>
            )}
          ...
          <button
            ...
            className="btn-outline flex items-center gap-2 py-2 px-4 rounded-xl text-xs font-semibold text-red-600 hover:bg-red-50 hover:border-red-100"
          >
        ...
        <div className="flex flex-wrap gap-2 mt-6 border-b border-slate-200 pb-4">
          {[
            {
              id: "hardware",
              label: `Hardware Products (${hardwareProducts.length})`,
              icon: FaLaptop,
            },
            {
              id: "software",
              label: `Software Products (${softwareProducts.length})`,
              icon: FaLayerGroup,
            },
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Add desktop/laptop products with multiple images, red MRP, green selling price,
                  features list and Flipkart-style specs.
                </p>
              </div>
              <Link
                to="/admin-add-product"
                search={{ type: "hardware" }}
                className="btn-primary flex items-center gap-2 py-2.5 px-5 rounded-xl text-xs font-bold shadow-md"
              >
                  ...
                  {hardwareProducts.map((p, idx) => {
                    return (
                      <tr key={p.id || idx} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3.5 px-3">
                          {p.image ? (
                            <img
                              src={p.image}
                              alt={p.name}
                              className="h-10 w-12 object-contain rounded border border-slate-200 bg-slate-50"
                            />
                          ) : (
                        ...
                        <td className="py-3.5 px-3 text-emerald-600 font-extrabold text-base">
                          {p.price}
                        </td>
                        <td className="py-3.5 px-3 text-red-500 line-through text-xs font-semibold">
                          {p.originalPrice || "-"}
                        </td>
                        <td className="py-3.5 px-3">
                          {p.discount ? (
                        ...
                        <td className="py-3.5 px-3 text-right space-x-3 whitespace-nowrap">
                          <Link
                            to="/admin-add-product"
                            search={{ id: p.id }}
                            className="btn-outline py-1.5 px-3 text-xs font-semibold rounded-lg inline-flex items-center gap-1.5"
                          >
                            <FaEdit /> Edit Specs & Photos
                          </Link>
                          <button
                            onClick={() => handleDeleteProduct(p)}
                            className="text-red-600 hover:text-red-800 p-1.5 rounded hover:bg-red-50 text-xs font-semibold inline-flex items-center gap-1"
                          >
                            <FaTrash />
                          </button>
                        </td>
          ...
          <div className="mt-6 bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Manage genuine software packages, licensing tiers, MRP vs offer prices.
                </p>
              </div>
              <Link
                to="/admin-add-product"
                  ...
                  {softwareProducts.map((p, idx) => {
                    ...
                    return (
                      <tr key={p.id || idx} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-3.5 px-3 text-red-500 line-through text-xs font-medium">
                          {p.originalPrice || "-"}
                        </td>
                        <td className="py-3.5 px-3 text-emerald-600 font-bold">{p.price}</td>
                        <td className="py-3.5 px-3 text-emerald-600 font-bold text-xs">
                          {p.discount || "-"}
                        </td>
                        <td className="py-3.5 px-3 text-right space-x-3 whitespace-nowrap">
                          <Link
          ...
          <div className="mt-6 bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
            <div className="flex justify-between items-center pb-6 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-800">
                BUSY Software Price Matrix ({busyPricing.length})
              </h2>
              <button
                onClick={handleAddBusy}
                className="btn-primary flex items-center gap-2 py-2 px-4 rounded-xl text-xs font-semibold"
              >
                <FaPlus /> Add Variant
              </button>
            </div>
                        ...
                        <>
                          <td className="py-3 px-2">
                            <select
                              ...
                              onChange={(e) =>
                                setEditBusy({ ...editBusy, edition: e.target.value })
                              }
                              className="px-2 py-1.5 border border-slate-200 bg-white rounded text-xs"
                            >
                          ...
                          <td className="py-3 px-2">
                            <input
                              ...
                              onChange={(e) =>
                                setEditBusy({ ...editBusy, variant: e.target.value })
                              }
                          ...
                          <td className="py-3 px-2">
                            <input
                              ...
                              onChange={(e) =>
                                setEditBusy({ ...editBusy, activation: e.target.value })
                              }
                              className="px-2 py-1 border border-slate-200 rounded text-xs w-full"
                            />
                          </td>
                          <td className="py-3 px-2">
                            <input
                              ...
                              onChange={(e) =>
                                setEditBusy({ ...editBusy, renewal: e.target.value })
                              }
                              className="px-2 py-1 border border-slate-200 rounded text-xs w-full"
                            />
                          </td>
                          <td className="py-3 px-2 text-right space-x-2 whitespace-nowrap">
                            <button
                              onClick={() => handleSaveBusy(idx)}
                              className="text-green-600 hover:text-green-800 text-xs font-bold flex items-center gap-1 inline-flex"
                            >
                              <FaSave /> Save
                            </button>
                          </td>
        ...
        {activeTab === "tally" && (
          <div className="mt-6 bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
            <div className="flex justify-between items-center pb-6 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-800">
                Tally Prime Price Matrix ({tallyPricing.length})
              </h2>
              <button
                        ...
                        <>
                          <td className="py-3 px-2">
                            <input
                              ...
                              onChange={(e) =>
                                setEditTally({ ...editTally, variant: e.target.value })
                              }
                              className="px-2 py-1 border border-slate-200 rounded text-xs w-full"
                            />
                          </td>
                          <td className="py-3 px-2">
                            <input
                              ...
                              onChange={(e) =>
                                setEditTally({ ...editTally, activation: e.target.value })
                              }
                              className="px-2 py-1 border border-slate-200 rounded text-xs w-full"
                            />
                          </td>
                          <td className="py-3 px-2">
                            <input
                              ...
                              onChange={(e) =>
                                setEditTally({ ...editTally, renewal: e.target.value })
                              }
                              className="px-2 py-1 border border-slate-200 rounded text-xs w-full"
                            />
                          </td>
          ...
          <div className="bg-white rounded-3xl max-w-4xl w-full p-6 md:p-8 shadow-2xl my-8 border-none">
            <div className="flex justify-between items-center pb-3">
              ...
              <button
                ...
                className="h-8 w-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-600 text-sm transition-colors border-none"
              >
                <FaTimes />
              </button>
            </div>
            ...
            <div className="mt-4 grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-sm font-bold text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                >
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-sm font-semibold text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                >
              ...
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. LENOVO IDEACENTRE A340 24IWL / DELL LATITUDE 5490"
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-sm font-bold tracking-wide uppercase outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-100/80 text-slate-900 rounded-2xl text-sm font-bold outline-none focus:bg-white focus:ring-2 focus:ring-slate-300 transition-all border-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-sm font-semibold outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                />
              </div>

              <div>
                <label className="text-xs font-bold text-slate-600 mb-1 block">
                  Specs Highlight Badge
                </label>
                <input
                  type="text"
                  value={editProduct.tag}
                  onChange={(e) => setEditProduct({ ...editProduct, tag: e.target.value })}
                  placeholder="e.g. Core i3 10th · 8GB · 512GB SSD"
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-sm font-semibold outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-sm outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    placeholder="Refurbished - Grade A+"
                    className="px-3.5 py-2.5 bg-slate-100/80 rounded-2xl text-xs font-semibold outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                  />
                  <input
                    type="text"
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  placeholder={`https://images.unsplash.com/photo-1527443224154-c4a3942d3acf...\nhttps://images.unsplash.com/photo-1587831990711...`}
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-xs font-mono outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                />
                <div className="mt-2 flex flex-wrap items-center gap-1.5">
                  ...
                  {PRESET_IMAGES.map((img) => (
                    <button
                      ...
                      className="px-2.5 py-1 bg-slate-100 hover:bg-primary hover:text-white rounded-full text-[10px] font-semibold text-slate-600 transition-colors border-none"
                    >
                      + {img.label}
                    </button>
                  ))}
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  placeholder={`23.8-inch Full HD Borderless IPS Display\nIntel Core i3 10th Gen Processor\n8GB DDR4 RAM with 512GB NVMe SSD`}
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-xs outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                />
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Overview of product performance, suitability for accounting and productivity..."
                  className="w-full px-4 py-2.5 bg-slate-100/80 rounded-2xl text-sm outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all border-none"
                />
              </div>
              ...
              <div className="sm:col-span-2 pt-2">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl space-y-3">
                  {Object.entries(editProduct.specs || {}).map(([sectionName, sectionData]) => (
                    <div
                      key={sectionName}
                      className="bg-white rounded-2xl p-3 shadow-sm space-y-1.5"
                    >
                      <div className="font-bold text-xs text-slate-800">{sectionName}</div>
                        ...
                        {typeof sectionData === "object" && sectionData !== null ? (
                          Object.entries(sectionData).map(([k, v]) => (
                            ...
                            >
                              ...
                              <button
                                type="button"
                                onClick={() => handleRemoveSpecItem(sectionName, k)}
                          ...
                          <div className="flex items-center justify-between text-xs py-1 px-2.5 bg-slate-50 rounded-xl">
                            <span className="font-semibold text-slate-600 w-1/3">
                              {sectionName}:
                            </span>
                            ...
                            <button
                              type="button"
                              onClick={() => handleRemoveSpecItem("General", sectionName)}
                              className="text-slate-400 hover:text-red-600 font-bold text-xs"
                            >
                              ✕
                            </button>
                  ...
                  <div className="pt-1 flex flex-wrap sm:flex-nowrap gap-2 bg-white p-3 rounded-2xl shadow-sm">
                    <select
                      ...
                      className="px-3 py-2 bg-slate-100 rounded-xl text-xs font-semibold outline-none border-none shrink-0"
                    >
                    ...
                    <input
                      type="text"
                      placeholder="Property (RAM)"
                      value={specKey}
                      onChange={(e) => setSpecKey(e.target.value)}
                      className="px-3 py-2 bg-slate-100 rounded-xl text-xs outline-none border-none flex-1"
                    />
                    <input
                      type="text"
                      placeholder="Value (8 GB DDR4)"
                      value={specVal}
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setIsModalOpen(false)}
                className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl text-xs font-bold transition-colors border-none"
              >
                Cancel
              </button>
              <button
                ...
                className="btn-primary py-2.5 px-6 rounded-2xl text-xs font-bold shadow-lg shadow-primary/20"
              >
