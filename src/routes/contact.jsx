  ...
  head: () => ({
    meta: [
      { title: "Contact VM Solutiions — Get a Free IT Consultation" },
      {
        name: "description",
        content:
          "Call, WhatsApp or write to VM Solutiions. Free consultation and same-day response guaranteed.",
      },
    ],
  }),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && resData.success !== false) {
        ...
        setStatus({
          type: "error",
          message: resData.error || "Failed to submit message. Please try again.",
        });
      }
    } catch (err) {
      clearTimeout(timer);
        ...
        <div className="container-x grid lg:grid-cols-[1fr_1.4fr] gap-10">
          ...
          <div className="space-y-6">
            <SectionHeading
              ...
              title={
                <>
                  Talk to <span className="text-primary">Our Experts</span>
                </>
              }
              subtitle="We reply within one business hour. Call or WhatsApp us any time for urgent support."
            />
            ...
            <div className="space-y-4 pt-2">
              {[
                ...
                <a
                  key={item.title}
                  href={item.href || "#"}
                  className="flex items-start gap-4 p-5 rounded-2xl bg-white border border-border hover:border-primary/40 transition-colors shadow-sm"
                >
          ...
          <div className="bg-white rounded-3xl p-6 md:p-10 shadow-lg border border-border">
            <TextReveal as="h3" className="text-2xl font-bold text-foreground">
              Send us a Message
            </TextReveal>
            ...
            {status && (
              ...
              >
                {status.type === "success" && (
                  <FaCheckCircle className="shrink-0 text-lg text-green-600" />
                )}
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label
                    htmlFor="fullName"
                    className="block text-xs font-bold uppercase tracking-wider mb-2 text-foreground/80"
                  >
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-border bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 pointer-events-auto select-text cursor-text"
                  />
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="e.g. Tally Prime / Hardware"
                    className="w-full rounded-xl border border-border bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 pointer-events-auto select-text cursor-text"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-border bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 resize-y pointer-events-auto select-text cursor-text"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn-primary w-full justify-center text-sm py-3.5 mt-2 disabled:opacity-50 cursor-pointer"
              >
      ...
      <section className="pb-24">
        <div className="container-x">
          <div className="rounded-3xl border border-border shadow-sm p-8 bg-white flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <a
              href="https://maps.google.com/?q=Phool+Bagh+Bhiwadi+Alwar+Rajasthan"
              target="_blank"
              rel="noreferrer"
              className="btn-outline flex items-center gap-2 shrink-0 py-3 px-6 text-sm"
            >
