...
import { FaCheckCircle, FaBullseye, FaEye, FaHeart, FaArrowRight, FaUserTie, FaLaptopCode, FaCloud, FaCalculator, FaShieldAlt } from "react-icons/fa";
...
export const Route = createFileRoute("/about")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Learn about VM Solutiions — Founded by Vishal Singh on 19 Feb 2026." },
...
function AboutPage() {
  const SPECIALITIES = [
    { icon: FaCalculator, title: "Tally & BUSY Accounting", desc: "Expert setup, licensing, data migration & compliance." },
    { icon: FaCloud, title: "Cloud & AWS Hosting", desc: "Enterprise cloud hosting, VPS & Tally on AWS solutions." },
    { icon: FaLaptopCode, title: "Web & Software Solutions", desc: "Custom web development, portal engineering & UI/UX." },
    { icon: FaShieldAlt, title: "IT Infrastructure Strategy", desc: "End-to-end IT consulting, hardware sales & 24/7 support." },
  ];
    ...
    <>
      ...
      <section className="reveal py-24">
        <div className="container-x grid lg:grid-cols-2 gap-14 items-center">
          ...
          <div>
            <SectionHeading center={false} eyebrow="Who We Are" title={<>Your Long-term <span className="text-primary">IT Partner</span></>} subtitle="Founded on 19 Feb 2026 by Vishal Singh, VM Solutiions has grown into one of India's most trusted IT partners."/>
      ...
      <section className="reveal py-20 bg-surface border-y border-border">
        <div className="container-x">
          <SectionHeading eyebrow="Company Leadership" title={<>Meet Our <span className="text-primary">Founder</span></>} subtitle="Guiding VM Solutiions towards technology excellence and client satisfaction."/>

          <div className="mt-12 max-w-4xl mx-auto bg-white border border-border p-8 md:p-10 rounded-3xl shadow-[var(--shadow-soft)]">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              <div className="h-20 w-20 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center text-primary text-3xl shrink-0">
                <FaUserTie />
              </div>
              <div className="flex-1 text-center sm:text-left">
                <span className="inline-block text-[11px] font-extrabold uppercase tracking-wider text-primary bg-accent px-3 py-1 rounded-full border border-primary/20">
                  Founder & Managing Director
                </span>
                <h3 className="text-2xl sm:text-3xl font-bold font-display text-foreground mt-2">Vishal Singh</h3>
                <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
                  Driving strategic vision, corporate IT solutions, software licensing, cloud architecture, and client growth at VM Solutiions.
                </p>

                {/* Specialities Grid */}
                <div className="mt-8 pt-6 border-t border-border">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-primary mb-4">Core Specialities & Expertise</h4>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {SPECIALITIES.map((spec) => (
                      <div key={spec.title} className="flex items-start gap-3 p-3.5 rounded-xl bg-surface border border-border/80">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-muted-foreground mt-0.5 leading-snug">{spec.desc}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <section className="reveal py-20">
        <div className="container-x grid md:grid-cols-3 gap-6">
          {[
            ...
            <div key={c.title} className="bg-white rounded-2xl p-8 shadow-[var(--shadow-soft)] border border-border">
