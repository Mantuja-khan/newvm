...
export const Route = createFileRoute("/software")({
    head: () => ({
        meta: [
            ...
            { name: "description", content: "Check complete Tally Prime, BUSY software editions, and Tally on AWS Cloud user-based pricing plans with 1 year upgrade and support." },
        ],
    }),
    ...
    return (<>
      ...
      <section className="reveal py-16 bg-slate-50/50">
        <div className="container-x">
          <SectionHeading eyebrow="Authorized Sales & Cloud Partner" title={<>Tally Prime, BUSY & <span className="text-primary">Tally on AWS Cloud</span></>} subtitle="Genuine accounting software licenses, user-based Tally-on-AWS cloud hosting plans, and BUSY software price matrix."/>
          ...
          <div className="mt-8 flex justify-center">
            <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm inline-flex gap-2">
              <button onClick={() => setActiveTab("catalog")} className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === "catalog"
            ? "bg-slate-900 text-white shadow-md"
            : "text-slate-600 hover:bg-slate-100"}`}>
                Products Catalog ({filteredProducts.length})
              </button>
              <button onClick={() => setActiveTab("pricing-tables")} className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === "pricing-tables"
            ? "bg-slate-900 text-white shadow-md"
            : "text-slate-600 hover:bg-slate-100"}`}>
          ...
          {activeTab === "catalog" ? (<>
              ...
              <div className="mt-8 flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
            ...
            ].map((c) => (<button key={c.id} onClick={() => setSelectedCategory(c.id)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${selectedCategory === c.id
                ...
                <div className="relative w-full md:w-72">
                  ...
                  <input type="text" placeholder="Search Tally, AWS 1 User, BUSY..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 text-xs border border-slate-200 rounded-xl focus:outline-none focus:border-primary bg-slate-50/50"/>
                </div>
              </div>

              {/* Product Grid */}
              {filteredProducts.length === 0 ? (<div className="mt-12 text-center py-16 bg-white rounded-2xl border border-slate-200">
                  <p className="text-slate-500 font-medium">No software products match your search criteria.</p>
                ...
                </div>) : (<div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredProducts.map((p) => {
                    ...
                    return (<div key={prodId} className="group bg-white rounded-2xl border border-slate-200 p-5 shadow-[0_2px_12px_-3px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_32px_-6px_rgba(0,0,0,0.08)] hover:border-emerald-500/40 transition-all flex flex-col justify-between cursor-pointer">
                        <div>
                          ...
                          <Link to="/software/$id" params={{ id: prodId }} className="block">
                            <div className="relative h-44 w-full bg-slate-50 rounded-xl overflow-hidden flex items-center justify-center p-4">
                              {p.image ? (<img src={p.image} alt={p.name} loading="lazy" className="h-24 w-auto object-contain transition-transform duration-300 group-hover:scale-105"/>) : (<div className="text-slate-300 text-xs font-semibold">Genuine License</div>)}
                              <span className="absolute top-3 left-3 text-[10px] font-extrabold uppercase tracking-wider text-slate-800 bg-white/90 backdrop-blur px-2.5 py-1 rounded-md border border-slate-200">
                                {p.tag}
                              </span>
                              {p.discount && (<span className="absolute top-3 right-3 text-[10px] font-extrabold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded">
                                  {p.discount}
                                </span>)}
                            </div>
                        ...
                        <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                          {/* Price Section */}
                          <Link to="/software/$id" params={{ id: prodId }} className="block">
                            <div className="text-[10px] font-semibold text-slate-400 uppercase">License / Cloud Price</div>
                            <div className="flex items-baseline gap-2">
                              <span className="text-lg font-black text-emerald-600">{p.price}</span>
                              {p.originalPrice && (<span className="text-xs text-red-400 line-through">{p.originalPrice}</span>)}
                            </div>
                          </Link>

                          {/* Enquire Button */}
                          <a href={`https://wa.me/${BRAND.phoneRaw}?text=${whatsappMsg}`} target="_blank" rel="noopener noreferrer" className="bg-[#1cd466] hover:bg-[#18c35b] text-white py-2.5 px-4 rounded-full font-extrabold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-sm transition-all active:scale-95">
        ...
        <div className="mt-8 space-y-10">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
                <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-slate-900 font-display">Tally Prime License Pricing Matrix</h3>
                    <p className="text-xs text-slate-500">Official license activation & annual TSS renewal rates</p>
                  </div>
                </div>
                <div className="overflow-x-auto mt-4">
                  <table className="w-full text-left border-collapse text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-800">
                      {TALLY_PRICING_TABLE.map((t, idx) => (<tr key={idx} className="hover:bg-slate-50/50">
                          <td className="py-3.5 px-4 font-bold text-slate-900">{t.variant}</td>
                          <td className="py-3.5 px-4 font-extrabold text-emerald-600">{t.activation}</td>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
                <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-slate-900 font-display">Tally on AWS Cloud Pricing (Per User Base)</h3>
                    <p className="text-xs text-slate-500">Access Tally from anywhere on Mac, PC or Mobile with 99.9% uptime</p>
                  </div>
                </div>
                <div className="overflow-x-auto mt-4">
                  <table className="w-full text-left border-collapse text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-800">
                      {AWS_CLOUD_PRICING_TABLE.map((aws, idx) => (<tr key={idx} className="hover:bg-slate-50/50">
                          <td className="py-3.5 px-4 font-bold text-slate-900">{aws.users}</td>
                          <td className="py-3.5 px-4 font-extrabold text-emerald-600">{aws.price}</td>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
                <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-slate-900 font-display">BUSY Accounting Full Price Matrix</h3>
                    <p className="text-xs text-slate-500">Complete rates for Basic, Standard, Enterprise, Blue, Saffron & Emerald editions</p>
                  </div>
                </div>
                <div className="overflow-x-auto mt-4">
                  <table className="w-full text-left border-collapse text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-800">
                      {BUSY_PRICING_TABLE.map((b, idx) => (<tr key={idx} className="hover:bg-slate-50/50">
                          <td className="py-3 px-4 font-bold text-slate-500">{b.edition}</td>
                          <td className="py-3 px-4 font-bold text-slate-900">{b.variant}</td>
                          <td className="py-3 px-4 font-extrabold text-emerald-600">{b.activation}</td>
                          <td className="py-3 px-4 font-semibold text-slate-600">{b.renewal}</td>
                        </tr>))}
                    </tbody>
                  </table>
                </div>
