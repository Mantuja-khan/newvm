...
function ServiceDetail() {
  ...
  return (
    <>
        ...
        <div className="container-x grid lg:grid-cols-[1fr_320px] gap-14">
          <div>
            ...
            <div className="mt-6 grid md:grid-cols-3 gap-4">
              {service.benefits.map((b: { title: string; text: string }) => (
                <div key={b.title} className="p-6 rounded-2xl bg-white border border-border">
      ...
      {service.slug === "web-development" && (
        <section className="reveal py-24 bg-surface">
          <div className="container-x">
            <SectionHeading
              eyebrow="Recent Work"
              title={<>Projects <span className="text-primary">We've Delivered</span></>}
      ...
      {service.slug === "tally-busy" && (
        <section className="reveal py-24 bg-surface scroll-mt-20" id="pricing">
          <div className="container-x">
            <SectionHeading
              eyebrow="Pricing Plans"
              title={<>Software Pricing <span className="text-primary">Charts</span></>}
              subtitle="Latest official licensing prices for BUSY and Tally Prime."
            />
            <SoftwarePricing />
          </div>
        </section>
...
function SoftwarePricing() {
  const [activeTab, setActiveTab] = useState<"busy" | "tally">("busy");
  ...
  return (
    <div className="mt-12 bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
      {/* Top Navigation Tabs */}
      <div className="flex border-b border-slate-100 bg-slate-50/50">
        {[
          ...
          <button
            ...
            className={`flex-1 py-4 text-sm font-semibold transition-all border-b-2 text-center ${
              activeTab === tab.id
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6">
              ...
              <div className="relative w-full md:w-64">
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-slate-50 rounded-lg text-xs border border-slate-200 focus:outline-none focus:border-primary focus:bg-white transition-all"
                />
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-50 text-sm">
                  {filteredBusy.length > 0 ? (
                    filteredBusy.map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/30 transition-colors">
                        <td className="py-3.5 px-2">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold ${
                            item.edition === "Basic" ? "bg-blue-50 text-blue-700" :
                        ...
                        <td className="py-3.5 px-2">
                          {item.activation === "Discontinued" ? (
                            <span className="text-slate-400 text-xs italic">Discontinued</span>
                          ) : (
                            <span className="font-semibold text-slate-900">{item.activation}</span>
                          )}
                        </td>
                        <td className="py-3.5 px-2">
                        ...
                        <td className="py-3.5 px-2 text-right">
                          <a
                            href={`https://wa.me/919358853990?text=${encodeURIComponent(`Hello VM Solutiions, I am interested in BUSY Software - Edition: ${item.edition}, Variant: ${item.variant} (Activation: ${item.activation}, Renewal: ${item.renewal}). Please share more details.`)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                  ...
                  {TALLY_PRICING.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="py-4 px-2 text-right">
                        <a
                          href={`https://wa.me/919358853990?text=${encodeURIComponent(`Hello VM Solutiions, I am interested in Tally Prime - Variant: ${item.variant} (Activation: ${item.activation}, Renewal: ${item.renewal}). Please share more details.`)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-xs font-bold text-primary hover:underline"
                        >
