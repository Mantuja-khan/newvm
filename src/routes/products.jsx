...
export const Route = createFileRoute("/products")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Genuine Tally & BUSY software and business cloud hosting plans at honest prices.",
      },
    ],
  }),
      ...
      <section className="reveal py-24">
        <div className="container-x">
          <SectionHeading
            ...
            title={
              <>
                Curated <span className="text-primary">IT Products</span>
              </>
            }
            subtitle="Genuine software and cloud hosting plans — priced honestly, backed by real support."
          />
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-2">
            {CATEGORIES.map((c) => (
              <a
                key={c.id}
                href={`#${c.id}`}
                className="px-5 py-2 rounded-full bg-slate-100 text-slate-700 text-sm font-semibold hover:bg-primary hover:text-white shadow-sm transition-colors"
              >
                {c.name}
              </a>
            ))}
          ...
          {CATEGORIES.map((cat) => (
            <div key={cat.id} id={cat.id} className="mt-16 scroll-mt-24">
              <h3 className="text-2xl font-bold flex items-center gap-2.5">
                <span className="h-6 w-1.5 bg-primary rounded-full inline-block"></span>
                {cat.name}
              </h3>
              <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {productList
                  ...
                  .map((p) => (
                    <div
                      key={p.name}
                      className="group bg-white rounded-2xl p-4 shadow-[0_2px_12px_-3px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_-4px_rgba(0,0,0,0.08)] transition-all flex flex-col justify-between"
                    >
                      <div>
                        {/* Product Image */}
                        <div className="relative h-44 w-full bg-slate-50 rounded-xl overflow-hidden flex items-center justify-center">
                          ...
                          <span className="absolute top-2 left-2 text-[10px] font-bold uppercase tracking-wider text-slate-600 bg-white px-2 py-0.5 rounded-full">
                            {p.tag}
                          </span>
                        </div>

                        {/* Title */}
                        <h4 className="text-base font-bold text-slate-800 leading-snug group-hover:text-primary transition-colors mt-2">
                          {p.name}
                        </h4>
