...
function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"hardware" | "software" | "busy" | "tally">("hardware");

  // Datasets
  const [products, setProducts] = useState<Product[]>([]);
  const [busyPricing, setBusyPricing] = useState<BusyPrice[]>([]);
  const [tallyPricing, setTallyPricing] = useState<TallyPrice[]>([]);
  ...
  const [editingBusyIndex, setEditingBusyIndex] = useState<number | null>(null);
  const [editBusy, setEditBusy] = useState<BusyPrice>({ edition: "Basic", variant: "", activation: "", renewal: "" });

  const [editingTallyIndex, setEditingTallyIndex] = useState<number | null>(null);
  const [editTally, setEditTally] = useState<TallyPrice>({ variant: "", activation: "", renewal: "" });

  const [dbStatus, setDbStatus] = useState<{ connected: boolean; statusText: string; host: string } | null>(null);
  ...
  // PRODUCTS OPERATIONS
  const saveProductsToBackend = async (newProducts: Product[]) => {
    try {
  ...
  const handleOpenAddModal = (productType: "hardware" | "software") => {
  ...
  const handleOpenEditModal = (product: Product, originalIndex: number) => {
  ...
  const handleRemoveSpecItem = (section: string, key: string) => {
    const currentSpecs = { ...(editProduct.specs || {}) };
  ...
  const savePricingToBackend = async (busy: BusyPrice[], tally: TallyPrice[]) => {
    try {
  ...
  const handleSaveBusy = (idx: number) => {
  ...
  const handleDeleteBusy = (idx: number) => {
  ...
  const handleSaveTally = (idx: number) => {
  ...
  const handleDeleteTally = (idx: number) => {
    if (confirm("Are you sure you want to delete this variant?")) {
                    ...
                    return (
                      <tr key={p.id || idx} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3.5 px-3">
                          {p.image ? (
                            <img src={p.image} alt={p.name} className="h-10 w-12 object-contain rounded border border-slate-200 bg-slate-50" />
                          ) : (
                  ...
                  {softwareProducts.map((p, idx) => {
                    ...
                    return (
                      <tr key={p.id || idx} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3.5 px-3">
                          {p.image ? (
                            <img src={p.image} alt={p.name} className="h-8 w-8 object-contain rounded" />
                          ) : (
                ...
                <div className="mt-2 flex flex-wrap items-center gap-1.5">
                  ...
                  {PRESET_IMAGES.map((img) => (
                    <button
                      key={img.label}
                      type="button"
                      onClick={() => setImagesText((prev) => (prev ? prev + "\n" + img.url : img.url))}
              ...
              <div className="sm:col-span-2 pt-2">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl space-y-3">
                  {Object.entries(editProduct.specs || {}).map(([sectionName, sectionData]) => (
                    <div key={sectionName} className="bg-white rounded-2xl p-3 shadow-sm space-y-1.5">
                      ...
                      <div className="space-y-1">
                        {typeof sectionData === "object" && sectionData !== null ? (
                          Object.entries(sectionData).map(([k, v]) => (
                            <div key={k} className="flex items-center justify-between text-xs py-1 px-2.5 bg-slate-50 rounded-xl">
