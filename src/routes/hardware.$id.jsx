...
function HardwareDetailPage() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            ...
            const found = PRODUCTS.find((p) => p.id === id || p.name.toLowerCase().replace(/[^a-z0-9]+/g, "-") === id);
            if (found)
                setProduct(found);
            setLoading(false);
        };
    ...
    if (loading) {
        return (<div className="min-h-[60vh] flex items-center justify-center py-20 bg-slate-50">
    ...
    if (!product) {
        return (<div className="min-h-[60vh] flex flex-col items-center justify-center py-20 px-4 bg-slate-50">
    ...
    const relatedProducts = PRODUCTS.filter((p) => (p.cat === product.cat || p.type === product.type) && p.name !== product.name).slice(0, 3);
    ...
    return (<>
      ...
      <section className="py-12 bg-slate-50">
        <div className="container-x">
          <Link to="/hardware" className="inline-flex items-center gap-2 text-xs font-bold text-slate-600 hover:text-slate-900 transition-colors mb-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 md:p-8 grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 flex flex-col justify-between">
              <div>
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 p-4 flex items-center justify-center relative min-h-[350px] shadow-inner">
                  <img src={activeImage} alt={product.name} className="max-h-[320px] w-auto object-contain transition-all duration-300"/>

                  {product.discount && (<span className="absolute top-3 right-3 text-xs font-extrabold text-slate-800 bg-slate-100 px-2.5 py-1 rounded-full">
                      {product.discount}
                    </span>)}
                </div>

                {/* Multiple Image Thumbnails Gallery Selector */}
                {galleryImages.length > 1 && (<div className="mt-4 flex items-center justify-center gap-3 overflow-x-auto py-2">
                    {galleryImages.map((imgUrl, idx) => (<button key={idx} type="button" onClick={() => setSelectedImageIndex(idx)} className={`h-16 w-16 rounded-lg border-2 p-1 bg-white overflow-hidden transition-all shrink-0 ${selectedImageIndex === idx
                    ? "border-slate-800"
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <a href={`https://wa.me/${BRAND.phoneRaw}?text=${whatsappMsg}`} target="_blank" rel="noopener noreferrer" className="bg-slate-900 hover:bg-slate-800 text-white py-3.5 px-4 rounded-xl font-extrabold text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-md active:scale-[0.98] text-center">
                  <FaWhatsapp className="text-lg"/> SEND ENQUIRY
                </a>

                <button type="button" onClick={() => setQuoteModalOpen(true)} className="bg-slate-800 hover:bg-slate-700 text-white py-3.5 px-4 rounded-xl font-extrabold text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-md active:scale-[0.98]">
            ...
            <div className="lg:col-span-7">
              ...
              {product.rating && (<div className="flex items-center gap-3 mt-3">
              ...
              <div className="mt-5 p-4 bg-slate-50 border border-slate-200/80 rounded-xl">
                ...
                <div className="flex items-baseline gap-3 mt-1">
                  ...
                  {product.originalPrice && (<span className="text-lg font-bold text-slate-400 line-through">
                      {product.originalPrice}
                    </span>)}
                  {product.discount && (<span className="text-xs font-bold text-slate-800 bg-slate-200 px-2.5 py-1 rounded">
                      {product.discount}
                    </span>)}
                </div>
              </div>
              ...
              <div className="mt-6">
                ...
                <ul className="mt-3 space-y-2">
                  {displayFeatures.map((feat, idx) => (<li key={idx} className="flex items-start gap-2 text-xs text-slate-700 font-medium">
              ...
              {product.desc && (<div className="mt-6 border-t border-slate-200 pt-5">
              ...
              {Object.keys(parsedSpecs).length > 0 && (<div className="mt-8 border-t border-slate-200 pt-6">
                  ...
                  <div className="mt-4 border border-slate-200 rounded-xl overflow-hidden shadow-sm bg-white">
                    <table className="w-full text-left text-xs divide-y divide-slate-100">
                      <tbody>
                        {Object.entries(parsedSpecs).map(([k, v]) => (<tr key={k} className="hover:bg-slate-50/50">
          ...
          {relatedProducts.length > 0 && (<div className="mt-16 border-t border-slate-200 pt-10">
              ...
              <div className="mt-6 grid sm:grid-cols-3 gap-6">
                ...
                const rpWhatsapp = encodeURIComponent(`Hello VM Solutiions, I am interested in purchasing ${rp.name} (${rp.price}). Please share details.`);
                return (<div key={rpId} className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                      <div>
                        ...
                        <Link to="/hardware/$id" params={{ id: rpId }}>
                          <div className="h-36 bg-white rounded-lg p-2 flex items-center justify-center">
                            {rp.image ? (<img src={rp.image} alt={rp.name} className="h-full w-auto object-contain"/>) : (<span className="text-xs text-slate-400">No Image</span>)}
                          </div>
                      ...
                      <a href={`https://wa.me/${BRAND.phoneRaw}?text=${rpWhatsapp}`} target="_blank" rel="noopener noreferrer" className="mt-4 bg-[#1cd466] hover:bg-[#18c35b] text-white py-2 px-3 rounded-lg text-xs font-bold uppercase flex items-center justify-center gap-1.5">
      ...
      {/* QUOTE MODAL */}
      {quoteModalOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 relative">
            <button onClick={() => setQuoteModalOpen(false)} className="absolute top-4 right-4 h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-600">
              <FaTimes />
            </button>

            {quoteSubmitted ? (<div className="py-8 text-center">
              ...
              </div>) : (<form onSubmit={handleQuoteSubmit}>
                ...
                <div className="mt-4 space-y-3">
                  <div>
                    ...
                    <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Rahul Sharma" className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-xl text-xs"/>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-600">Phone Number *</label>
                    <input type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="9876543210" className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-xl text-xs"/>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-600">Email Address *</label>
                    <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="rahul@company.com" className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-xl text-xs"/>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-600">Custom Requirements / Quantity</label>
                    <textarea rows={2} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="e.g. Need 5 units for office accounting..." className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-xl text-xs"/>
                  </div>
                </div>
