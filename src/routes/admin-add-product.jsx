...
function AddProductPage() {
  ...
  const [product, setProduct] = useState({
    ...
    inStock: true,
    specs:
      "Processor: Intel Core i3 10th Gen, RAM: 8GB DDR4, Storage: 512GB NVMe SSD, Display: 23.8 inch FHD",
  });
      ...
      const fetchEditProduct = async () => {
        try {
          ...
          if (res.ok) {
            ...
            setProduct(data);
            setImagesInput(
              Array.isArray(data.images) ? data.images.join(", ") : data.images || data.image || "",
            );
            setFeaturesInput(
  ...
  const handleSubmit = async (e) => {
    ...
    const finalProduct = {
      ...
      id:
        product.id ||
        `${product.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${Date.now().toString(36)}`,
    };
    try {
      ...
      <div className="container-x max-w-3xl">
        <div className="flex items-center justify-between gap-4 pb-6">
          <div>
            <Link
              to="/admin"
              className="inline-flex items-center gap-2 text-xs font-bold text-slate-600 hover:text-slate-900 mb-2 bg-slate-200/60 px-3.5 py-1.5 rounded-full transition-colors"
            >
        ...
        <form
          ...
          className="bg-white rounded-3xl p-6 md:p-8 shadow-md space-y-6"
        >
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-slate-700 mb-1 block">
                Product Photo URLs (Comma-separated)
              </label>
              <input
                type="text"
                value={imagesInput}
            ...
            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-slate-700 mb-1 block">
                Technical Specifications (Comma-separated key:value pairs)
              </label>
              <textarea
                rows={3}
                value={specsInput}
