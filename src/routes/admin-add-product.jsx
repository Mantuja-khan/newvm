    ...
    return (<div className="min-h-screen bg-slate-50 py-10">
      <div className="container-x max-w-3xl">
        <div className="flex items-center justify-between gap-4 pb-6">
          ...
          <button type="button" onClick={handleSubmit} disabled={saving} className="btn-primary py-3 px-6 rounded-2xl font-bold text-xs shadow-md flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 md:p-8 shadow-md space-y-6">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <select value={product.type} onChange={(e) => setProduct({ ...product, type: e.target.value })} className="w-full px-4 py-3 bg-slate-100/80 rounded-2xl text-sm font-semibold text-slate-900 outline-none border-none">
            ...
            <div>
              ...
              <select value={product.cat} onChange={(e) => setProduct({ ...product, cat: e.target.value })} className="w-full px-4 py-3 bg-slate-100/80 rounded-2xl text-sm font-semibold text-slate-900 outline-none border-none">
            ...
            <div className="sm:col-span-2">
              ...
              <input type="text" required value={product.name} onChange={(e) => setProduct({ ...product, name: e.target.value })} placeholder="e.g. LENOVO IDEACENTRE A340 24IWL" className="w-full px-4 py-3 bg-slate-100/80 rounded-2xl text-sm font-bold text-slate-900 uppercase outline-none border-none"/>
            </div>

            <div>
              <label className="text-xs font-bold text-slate-700 mb-1 block">Selling Price *</label>
              <input type="text" required value={product.price} onChange={(e) => setProduct({ ...product, price: e.target.value })} placeholder="e.g. ₹35,000" className="w-full px-4 py-3 bg-slate-100/80 text-slate-900 rounded-2xl text-sm font-bold outline-none border-none"/>
            </div>

            <div>
              <label className="text-xs font-bold text-slate-700 mb-1 block">Original Price / MRP</label>
              <input type="text" value={product.originalPrice || ""} onChange={(e) => setProduct({ ...product, originalPrice: e.target.value })} placeholder="e.g. ₹40,000" className="w-full px-4 py-3 bg-slate-100/80 text-slate-900 rounded-2xl text-sm font-bold outline-none border-none"/>
            </div>

            <div>
              <label className="text-xs font-bold text-slate-700 mb-1 block">Discount Tag</label>
              <input type="text" value={product.discount || ""} onChange={(e) => setProduct({ ...product, discount: e.target.value })} placeholder="e.g. 13% Off" className="w-full px-4 py-3 bg-slate-100/80 text-slate-900 rounded-2xl text-sm outline-none border-none"/>
            </div>

            <div>
              <label className="text-xs font-bold text-slate-700 mb-1 block">Brand Name</label>
              <input type="text" value={product.brand || ""} onChange={(e) => setProduct({ ...product, brand: e.target.value })} placeholder="e.g. Lenovo, Dell, Tally" className="w-full px-4 py-3 bg-slate-100/80 text-slate-900 rounded-2xl text-sm outline-none border-none"/>
            </div>

            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-slate-700 mb-1 block">Product Photo URLs (Comma-separated)</label>
              <input type="text" value={imagesInput} onChange={(e) => setImagesInput(e.target.value)} placeholder="https://example.com/photo1.jpg, https://example.com/photo2.jpg" className="w-full px-4 py-3 bg-slate-100/80 text-slate-900 rounded-2xl text-sm outline-none border-none"/>
            </div>

            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-slate-700 mb-1 block">Highlights & Features (Comma-separated)</label>
              <textarea rows={2} value={featuresInput} onChange={(e) => setFeaturesInput(e.target.value)} placeholder="23.8-inch Full HD Display, Intel Core i3 10th Gen, 8GB DDR4 RAM, 512GB SSD" className="w-full px-4 py-3 bg-slate-100/80 text-slate-900 rounded-2xl text-sm outline-none border-none"/>
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <textarea rows={3} value={specsInput} onChange={(e) => setSpecsInput(e.target.value)} placeholder="Processor: Intel Core i3 10th Gen, RAM: 8GB DDR4, Storage: 512GB NVMe SSD, Display: 23.8 inch FHD" className="w-full px-4 py-3 bg-slate-100/80 text-slate-900 rounded-2xl text-sm outline-none border-none"/>
            </div>

            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-slate-700 mb-1 block">Description</label>
              <textarea rows={2} value={product.desc} onChange={(e) => setProduct({ ...product, desc: e.target.value })} placeholder="Brief description of product features and usage..." className="w-full px-4 py-3 bg-slate-100/80 text-slate-900 rounded-2xl text-sm outline-none border-none"/>
            </div>
          </div>
          ...
          <div className="pt-4 flex items-center justify-between gap-4">
            ...
            <button type="submit" disabled={saving} className="btn-primary py-3 px-8 rounded-2xl font-bold text-xs shadow-md">
