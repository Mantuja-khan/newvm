      ...
      <section className="reveal py-24">
        <div className="container-x grid lg:grid-cols-[1fr_320px] gap-14">
          <div>
            ...
            {service.image && (
              <div className="mt-8 rounded-2xl overflow-hidden border border-border/60 max-h-[440px] bg-slate-50/50 flex items-center justify-center p-4">
                <img src={service.image} alt={service.title} className="w-full h-full max-h-[400px] object-contain mx-auto" />
              </div>
            )}
          ...
          <div className="container-x">
            ...
            <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {PROJECTS.map((p) => {
                ...
                if (p.url) {
                  return (
                    <a key={p.title} href={p.url} target="_blank" rel="noopener noreferrer" className="group block relative overflow-hidden rounded-2xl bg-white shadow-[var(--shadow-soft)] hover:shadow-[var(--shadow-card)] transition-all">
                      {CardContent}
                    </a>
                  );
                }
                return (
                  <div key={p.title} className="group relative overflow-hidden rounded-2xl bg-white shadow-[var(--shadow-soft)] hover:shadow-[var(--shadow-card)] transition-all">
                    {CardContent}
                  </div>
                );
              })}
